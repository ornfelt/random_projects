...
public class GpsUtils
{
    ...
    const double a = 637.0;         // WGS-84 Earth semimajor axis (m)
    const double b = 635.314245;     // Derived Earth semiminor axis (m)
    ...
    { 
        // Test only works with WGS-84 geodetic constants (see top of file)
        //Test();
        //Test2();
        // https://gist.github.com/govert/1b373696c9a27ff4c72a

        var latLA = 10.00000048;
        var lonLA = 10.11;
        var hLA = 200.702;

        double x0, y0, z0;
        GeodeticToEcef(latLA, lonLA, hLA, out x0, out y0, out z0);
        Console.WriteLine("x0: " + x0 + ", y0: " + y0 + ", z0: " + z0);


        //public static void EcefToGeodetic(double x, double y, double z,
                                //out double lat, out double lon, out double h)
        double xTest = 812.2148876917684; 
        double yTest = 144.8237677007498;
        double zTest = 144.88952033192325;

        double lat0, lon0, h0;
        EcefToGeodetic(xTest, yTest, zTest, out lat0, out lon0, out h0);
        Console.WriteLine("lat0: " + lat0 + ", lon0: " + lon0 + ", h0: " + h0);
